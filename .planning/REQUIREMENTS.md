...
- [x] **SES-03**: Existe historial de sesiones (la de hoy contra la de ayer). —
  Backend completo en 37-02: `GET /api/setters/dial-sessions`, mismo patrón
  de scope que `cold-call-metrics` (setter propio, supervisor scoped vía
  ...
  clavada en comentario + test). **Cerrado de punta a punta en 37-04**:
  tabla "Sesiones de discado" en `view-myperf` (`_mypLoadSessions` +
  `_sesHistoryRows`, agrupada Hoy/Ayer/fecha, setter efectivo explícito en
  modo "Ver como SDR"). Ver `.planning/phases/37-ses-sesion-discado/
  37-02-SUMMARY.md` y `37-04-SUMMARY.md`.
- [x] **SES-04**: Al cerrar se hace una sola pregunta sobre el estado del que
  marcó, opcional de responder y nunca por llamada. — Backend completo en
  37-02: `PATCH /api/setters/dial-sessions/:id`, whitelist
  `DIAL_SESSION_MOODS` (`bien/normal/costo/pesimo`), `''` borra y nunca
  bloquea el cierre, IDOR cerrado por `id`+`setterId` del actor en el mismo
  `.find()`. **Deja de ser write-only en 37-04**: la columna "Cómo la remó"
  del historial lee `mood` de vuelta (`SES_MOOD_LABELS`, un solo mapa
  compartido con los chips de cierre de 37-03; guion discreto si no se
  respondió, D-03). Ver `.planning/phases/37-ses-sesion-discado/
  37-02-SUMMARY.md` y `37-04-SUMMARY.md`.
