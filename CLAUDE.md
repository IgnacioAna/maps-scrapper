...
105. **COST waves construidas como BOTONES OPT-IN (2026-06-18, deployadas — gasto gateado al user)**:
    ...
    - **ÚNICO PENDIENTE del roadmap**: REFACTOR Phase 13 (`_leadStore` single-source-of-truth + optimistic sync + home "Hoy" + anatomía única de card). NO deployado a ciegas a propósito: toca TODAS las vistas del call center; un bug deja el core roto y el user sin poder testear. Hacer EN VIVO con el user, por partes (orden del PLAN: leadStore → card unificada → home Hoy → vistas-chip). Cache-buster actual `v=20260618j` (app.js), style.css `v=20260618a`.
