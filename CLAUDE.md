...
105. **COST waves construidas como BOTONES OPT-IN (2026-06-18, deployadas — gasto gateado al user)**:
    ...
    - **ÚNICO PENDIENTE de TODO el roadmap**: el `_leadStore` (single-source-of-truth + optimistic sync entre Power Dialer y lista Calls — queja c de Phase 13). Es el rewrite INVASIVO (rewirea `_pd.queue`/`_callsLeadsById`/`callsLeadsCache` + hooks de re-render en todas las vistas). NO deployado a ciegas a propósito: un bug deja el core roto y el user sin poder testear; los edge cases (dispositions concurrentes, paginación, autopilot, races del cache) se atrapan probando en uso real, no en preview. **Hacer EN VIVO con el user.** Todo lo demás del roadmap (Phases 6-17 + olas safe/cost) está DONE + deployado. Cache-buster `v=20260618k` (app.js), style.css `v=20260618a`.
