...
81. **PENDIENTE call recording (audio)**: hoy el audio se graba en browser (MediaRecorder, setter+lead) y se sube a `/api/telnyx/calls/:leadId/transcribe` (Whisper) pero **NO se persiste** — solo queda el transcript en el callLog. Persistir el audio en el Railway Volume tiene tradeoffs (crecimiento del volumen ~240KB/min, el pre-deploy NO respalda binarios, solo JSON). Decisión diferida al user: audio files persistidos vs. transcript-only (que ya existe). Si se hace: guardar bajo `DATA_DIR/recordings/`, ref en `entry.recordingFile`, endpoint de stream + player, y hacerlo opt-in por flag para no bloatear silenciosamente.
